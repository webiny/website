      ...
      return new _promise.default(function (resolve, reject) {
        if (resourceName in jsonStore) {
          ...
          req.onreadystatechange = function () {
            if (req.readyState == 4) {
              if (req.status === 200) {
                resolve(jsonStore[resourceName] = JSON.parse(req.responseText));
              } else {
                reject();
              }
            }
...
var queue = {
  ...
  getResourcesForPathname: function getResourcesForPathname(path, cb) {
    ...
    if (pathScriptsCache[path]) {
      ...
      var component;
      var json; // Load the component/json in parallel and call this function when
      // they're done loading. When both are loaded, we move on.

      var done = function done() {
        if (component && json) {
          pathScriptsCache[path] = {
            component: component,
            json: json,
            page: page
          };
      ...
      getResourceModule(page.componentChunkName, function (err, c) {
        if (err) {
      ...
      getResourceModule(page.jsonName, function (err, j) {
        if (err) {
