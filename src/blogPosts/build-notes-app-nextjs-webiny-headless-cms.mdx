...
Webiny is an open-source headless CMS that allows you to run your applications serverless. It is a GraphQL-based dynamical GraphQL API endpoint. To learn more about Webiny’s headless CMS, [check this guide and understand its unique features](https://www.webiny.com/why-webiny/).

Next.js is a React-based frontend web framework. It allows you to perform server-side rendering (SSR) operations at its core.

In this guide, we will build a Webiny content model for a notes app. This will give us an API that we will consume using the Next.js framework. So let’s dive in!
