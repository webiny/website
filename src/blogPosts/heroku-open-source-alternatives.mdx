---
slug: "blog/heroku-open-source-alternatives"
title: "Open source alternatives to Heroku"
...
date: 2022-08-26
featureImage: "./assets/heroku-open-source-alternatives/cover.png"
...
Heroku started as a platform offering hosting to exclusively Ruby-based websites. It's since expanded that option and now a host of "build packs" exist which can deploy infrastructure to Heroku in one click. This was likely the inspiration for similar "one click deploy" buttons that have been adopted by Netlify, Vercel and others.
...
Heroku wasn't only used for blogging: many people had used it to host backend services for small hobby apps that didn't make any money, or for proof-of-concept ideas so they could evaluate tools for their organizations to later use in production. 

If this is what you're looking for, there are a host of new services like [Fly.io](https://fly.io/) and [Render](https://render.com/) that offer free tiers.

However, we see another problem looming...
...
And whilst you could go ahead and setup an EC2 instance (similar to a virtual machine or VM), and configure it to hibernate in a similar fashion to what Heroku was doing, there's another opportunity here that we would like to highlight.

## Embrace Serverless

If you notice, the limits on EC2 instances and the like are time-based. You get a certain number of hours of free computing power in traditional "always-on" computing models.

However, other services are based on the number of invocations. Why not take adantage of these? 
...
If you were to build your solution using a combination of these, you could host it indefinitely. It wouldn't need special configuration to hibernate since that is it's natural state.
