...
On the GraphQL API side, we'd want to start by expanding the [`CarManufacturer` entity](https://github.com/webiny/webiny-js/blob/v5.9.0/packages/cli-plugin-scaffold-graphql-service/template/entities/TargetDataModels.ts#L15-L22) and our [GraphQL schema](https://github.com/webiny/webiny-js/blob/v5.9.0/packages/cli-plugin-scaffold-graphql-service/template/typeDefs.ts) with attributes and fields, respectively. Once we're done, we'd then continue by propagating the same changes on the to the Admin Area React application side, by adding new fields to the [`CarManufacturersForm`](https://github.com/webiny/webiny-js/blob/v5.9.0/packages/cli-plugin-scaffold-admin-app-module/template/views/TargetDataModelsForm.tsx#L50-L82) React component and updating GraphQL queries and mutations we already had in place.

Initially, the `CarManufacturer` entity only contains two fields: `title` and `description`, which we can set via the form that's part of the generated CRUD view in our Admin Area React application. So, for example, let's see how we can add the new `isPopular` boolean attribute to the entity, and then, enable logged-in administrators to modify its value, by adding a simple `Switch` form element.

#### GraphQL API

Starting from the GraphQL API side, let's open our [`CarManufacturers`](https://github.com/webiny/webiny-js/blob/v5.9.0/packages/cli-plugin-scaffold-graphql-service/template/entities/TargetDataModels.ts#L15-L22) entity and simply define the new attribute on it (parts of code removed for brevity):

```ts
// api/code/graphql/src/plugins/scaffolds/graphql/entities/CarManufacturers.ts
...
Notice how we're passing the [`CarManufacturerEntity`](#) interface upon instantiating the `Entity` class. Since the interface is essentially listing all attributes that our entity consists of, let's update it as well:

```ts
// api/code/graphql/src/plugins/scaffolds/graphql/types.ts
...
Finally, let's update our GraphQL `CarManufacturer`-related types in our schema, located in the [`typeDefs.ts`](https://github.com/webiny/webiny-js/blob/v5.9.0/packages/cli-plugin-scaffold-graphql-service/template/typeDefs.ts) file:

```ts
// api/code/graphql/src/plugins/scaffolds/graphql/CAR-MANUFACTURERS/typeDefs.ts
...
Note that if you've previously run the two shown [`webiny watch`](#) commands, all of the changes should be automatically redeployed into the cloud as you're making them.
...
In the [`CarManufacturersForm`](https://github.com/webiny/webiny-js/blob/v5.9.0/packages/cli-plugin-scaffold-admin-app-module/template/views/TargetDataModelsForm.tsx#L50-L82) form, let's add a new [`Switch`]() form element, which will enable logged-in users to mark a particular car manufacturer as popular or not popular (parts of code removed for brevity):
