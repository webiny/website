...
There is no doubt that End-to-End testing is hard, time-consuming, and comes with a bag of issues to solve. But, only if you're using the wrong tool for the job.
...
In this blog post, we'll cover 5 such mistakes to avoid when writing Cypress tests.
...
What that means is, we can't use syntax like `async-await` while working with them.
