...
tags: ["Serverless", "Lambda Layers"]
featureImage: "./assets/learn-how-to-use-lambda-layers/featureImage.png"
author: swapnil
date: 2022-05-02
---

The <ExternalLink href="https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-concepts.html#gettingstarted-concepts-layer">Lambda Layer</ExternalLink> is a true gem in the Lambda function realm. It provides a convenient way to package your libraries and other dependencies that can be used with the Lambda functions.
...
- AWS account - To use various AWS services, you will need an AWS account. Please follow [this link](https://aws.amazon.com/premiumsupport/knowledge-center/create-and-activate-aws-account/) to create an AWS account.
- AWS command line - We will use the AWS command line for creating Lambda Function and Lambda Layer. The AWS CLI installation guide is available [here](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html).
...
2. [Create two S3 buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html) - source and target. The target bucket name should be `<source-bucket-name>` plus suffixed by `-resized` .  For example, if the source bucket is named `mybucket` then the target bucket should be named as `mybucket-resized`. Please follow this naming convention because we will be using this naming logic in the Lambda function. 
...
Create the [execution role](https://docs.aws.amazon.com/lambda/latest/dg/lambda-intro-execution-role.html) that gives your Lambda function permissions to access AWS resources.
...
Here comes the special part of this tutorial. We will create the [sharp](https://sharp.pixelplumbing.com/install#aws-lambda) Lambda Layer in this section. 

1. Create a directory with the name `nodejs`.
2. Install the `sharp` dependency in the `nodejs` directory by running the following command:
...
The deployment package is a [.zip file archive](https://docs.aws.amazon.com/lambda/latest/dg/gettingstarted-package.html#gettingstarted-package-zip) containing your Lambda function code and its dependencies.
...
Please replace the role parameter `123456789012`with your [AWS account ID](https://docs.aws.amazon.com/general/latest/gr/acct-identifiers.html) and Layers ARN with the one we got in Step 4 (`LayerVersionArn`)**.**
...
1. Open the [Lambda page](https://console.aws.amazon.com/lambda/home) in the AWS console.
...
4. Create a trigger with the following properties:
    - **Select a trigger** – S3
    - **Bucket** - mybucket (your source bucket)
    - **Event type** – **PUT**
    - **Role name** – `lambda-s3-role`
...
In case you want to explore and learn more about best coding practices and code examples related to Lambda, you can check out the [Webiny code repository](https://github.com/webiny/webiny-js/). Webiny is an open-source enterprise-grade serverless CMS. 

If you have any questions related to this tutorial or have any feedback, please feel free to ping us on the [Community Slack](webiny.com/slack/)!

## Credits
As this tutorial aims to explain and demonstrate the use of Lambda Layer, we didn't reinvent the wheel and write a new Lambda function to create thumbnail images. The Lambda function code used in this article to create thumbnail images is taken from this <ExternalLink href="https://docs.aws.amazon.com/lambda/latest/dg/with-s3-tutorial.html">official AWS Lambda tutorial</ExternalLink>. Huge thanks to the authors of this great tutorial. 🙏🏻