...
AWS Lambda is awesome! The service allows us to easily deploy and run our own code, without worrying too much about the underlying infrastructure (when compared to pre-serverless). They essentially scale infinitely (with great power comes great responsibility), and can be connected with a bunch of services, like API Gateway, S3, AppSync, DynamoDB, etc.
...
Consider these two [512MB](/static/blogPosts/cwLambda512.png) RAM and [1024MB](./assets/5-tips-to-make-your-lambda-functions-run-faster-and-cheaper/cwLambda1024.png) RAM Lambda function CloudWatch logs. The billed durations from the logs are also outlined shown in the following chart:

![Snipaste_2020-10-14_19-40-57-BBB](https://user-images.githubusercontent.com/5121148/96137705-3eea7680-0efd-11eb-8ddb-a02e7f99158a.png)
![Snipaste_2020-10-14_19-40-10-BBB](https://user-images.githubusercontent.com/5121148/96137718-427dfd80-0efd-11eb-98b0-ba882d8a466b.png)

![Comparing function invocation duration with 512MB and 1024MB of RAM](./assets/5-tips-to-make-your-lambda-functions-run-faster-and-cheaper/lambda512vs1024.png)

So, what we can see is that with the 512MB of RAM Lambda function, the billed duration is often 200ms. But, with 1024MB of RAM, which is 2x more, the billed duration gets reduced to 100ms, which is 2x less. Even the initial invocation's duration (the cold start one's) got reduced from 1400ms to 700ms. 
...
Before bumping up RAM, do test your function with different payloads, and see how it'll behave with different RAM configurations.
...
But, based on that fact, the advice I wanted to give here is - watch out for function size. Try to use as few external packages as possible. Be aware that every package you include in your function can also bring additional dependencies with it, making it even worse. 

With that, also be careful about how you actually import packages. If there is a way to import just a specific functionality of the package, do that, instead of importing the entire code.

For example, when using the AWS SDK, which we often need to communicate with various AWS services, only import the actual clients you'll be using.
...
This approach can significantly affect the final bundle size.

And before we wrap this section up, I just wanted to mention there are a couple of tools I like to use in order to inspect what the 3rd party package is bringing to the table (or should I say - removing from the table).
...
The other one is the Webpack Visualizer, which gives you an awesome overview of all the packages that are included in your final production Webpack bundle. For example:

![Comparing function invocation duration with 512MB and 1024MB of RAM](./assets/5-tips-to-make-your-lambda-functions-run-faster-and-cheaper/webpackBundleGraph.png)
...
Configuring Webpack could be a bit daunting, but it's definitely worth it at the end of the day. This is what we, at Webiny, recognized very early, and that's why we've ensured that every new Webiny project offers Webpack bundling out of the box. Everything is pre-configured for you, so you can immediately jump to the code! 🙂
...
Yes, you can also invoke functions asynchronously, without waiting for the invocation to complete (using the InvocationType: "Event"), but, here we're talking more about synchronous function invocation, which is definitely needed in case we're interested in the actual response that the invoked function is returning.
...
![Comparing function invocation duration with 512MB and 1024MB of RAM](./assets/5-tips-to-make-your-lambda-functions-run-faster-and-cheaper/splitWorkBetweenLambdaFunctions1.png)
...
I won't go too much into the details here (feel free to check the Serverless Side Rendering - The Ultimate Guide article for that), but, what we are doing with these two Lambda functions is essentially splitting work and optimizing for cost. The "Web Server Lambda" is configured with minimal system resources, because its only task is to return cached SSR HTML, stored in the database. 

But, if there is no SSR HTML in the database, we need to generate it, which is actually an intensive process, and executing it with 128MB of RAM simply won't cut it. So, instead of just bumping the Web Server Lambda's RAM, we are invoking the "SSR Lambda" function, which contains a lot more system resources (for example - 2 GB RAM), and we're only paying for that when we actually need it. It would be a waste of money to have the "Web Server Lambda" configured with the same amount of resources, just to pull some data out of the database, most of the time. This way we can save a significant amount of money in the long run.

You can also do this if you have a Lambda function that's heavy in size, just because of a 3rd dependency that needs to be there, but still, isn't triggered that often.

For example, one of the apps that Webiny offers out of the box, the Page Builder app, has its own installation process, in which we basically download initial sample pages and images, and respectively, insert them into the database, and save them to user's S3 bucket. This process requires a couple of NPM packages for dealing with files, which can easily add a couple of hundreds of KBs to the overall function size.
...
![Comparing function invocation duration with 512MB and 1024MB of RAM](./assets/5-tips-to-make-your-lambda-functions-run-faster-and-cheaper/splitWorkBetweenLambdaFunctions2.png)
...
Doing this will decrease the function invocation duration and naturally, save us money along the way!

![Comparing function invocation duration with 512MB and 1024MB of RAM](./assets/5-tips-to-make-your-lambda-functions-run-faster-and-cheaper/niceMeme.jpg)

## 5. Reusing connections with Keep-Alive
Last but not least, if you're making HTTP requests within your Lambda function's code, it's useful to enable the "keep-alive" option on the default Node.js HTTP/HTTPS agent, because that avoids establishing the TCP connection on every request, and enables us to always reuse the existing one:
...
By default, the default Node.js HTTP/HTTPS agent creates a new TCP connection for every new request. To avoid the cost…docs.aws.amazon.com

The somehow shocking fact about establishing new TCP connections is that sometimes the actual establishment takes more time than the actual client operation we're trying to execute! So when you think about it, deciding whether this option should be enabled or not is almost a no-brainer.
...
A good article on the actual performance boost that this option introduces was already written by Yan Cui, in his Lambda optimization tip - enable HTTP keep-alive article. As we can see, without HTTP keep-alive, the duration of DynamoDB operations averaged around 33ms, while with the option enabled, the average duration dropped to 10ms! 

## Conclusion
Lambda functions to make our lives easier, by relieving us from thinking about the infrastructure and reducing the overall cost, but still, there are a couple of tricks that we need to know, in order to optimize the work even further.
