...
Serverless infrastructure is changing the way we architect applications and it’s also changing the way we code applications and work as developers. The development is moving away from our laptop and into the cloud. It’s the start of cloud-native development.
...
Another meaningful advantage is the **simplicity of debugging**. If your code, database, or network is not working, you access those components and try and find the problem. You attach debuggers, inspectors, and profilers along the way to gain additional insight.
...
When working in a larger organization the LDE can be **large and complex, or even scary**. These environments take a lot of time to get them up and running. When they break, you often need to run the whole setup from the ground up.
...
Many such examples are available, but the main takeaway is those services are not available inside an LDE. Serverless services in the majority of cases can only be consumed via an API contacting the service cloud provider.
