...
This is because these project applications represent React applications. In order to develop these, we don't need automatic re-deployment and logs forwarding features. Simply put, frontend development can still be performed locally. Simply said, frontend development can still be performed locally.

Once we're ready to deploy it to the actual cloud, we can do that via the [`webiny deploy`](https://www.webiny.com/docs/how-to-guides/deployment/deploy-your-project) command, described previously in the article.
