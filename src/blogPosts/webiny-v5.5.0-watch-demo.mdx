...
So once again, we asked ourselves - instead of starting watch sessions on particular packages in a Webiny project, wouldn't be just easier if we could initiate a watch session on a complete project application?
...
If not using the native Node.js debugger, usually we tend to place one or more `console.log` calls in our code and then run the application again (for example, a specific GraphQL query). And although in serverless environments this is possible, when it comes to actually reading the logged data, the developer experience has shown to be a bit cumbersome and really not that straight forward.
...
This is because, these project applications, essentially, represent frontend React applications, and in order to develop these, we don't need the shown automatic re-deployment and logs forwarding features. Simply said, frontend development can still be performed locally.

Once we reach a point in which we feel confident enough to deploy it to the actual cloud, then we can do that via the already seen [`webiny deploy`](https://www.webiny.com/docs/how-to-guides/deployment/deploy-your-project) command.

But, regarding everything else, we can continue using the same approach we used in the previous backend development section. For example, we could also start by simply scaffolding a new **Admin Area** module, completing the extra steps that are provided for us, and finally, running the shown `webiny watch` commands.
