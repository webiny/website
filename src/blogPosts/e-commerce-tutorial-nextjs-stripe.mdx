...
- Content Delivery API Access Token
  ...
  1.  Follow the <ExternalLink href="http://docs.webiny.com/docs/webiny-apps/headless-cms/features/access-tokens/">link</ExternalLink> here to create an access token
...
Now that we have all of the prerequisites out of the way, it's time to create our first <ExternalLink href="http://docs.webiny.com/docs/webiny-apps/headless-cms/features/content-modeling/">content model</ExternalLink> for the e-commerce website.
...
and pass the options to this, which is the `uri` → That means: `Where on the internet does your GraphQL URL exists?` 🚀
...
You can freely open a PR for the `goToProduct` function and make it your first PR @ <ExternalLink href="https://www.webiny.com/">Webiny</ExternalLink>. 👩🏻‍🚀
Head over to our <ExternalLink href="https://github.com/webiny/community">Community repository </ExternalLink> to open the PR 🚀

## 4. Next.js + Stripe to create the payment intents

In this tutorial we will use the Stripe Payment intents API with Next.js and
follow best practices as set out by Stripe and the industry.

### Prerequisites

Before we continue, we need to create a Stripe account, and make sure to navigate to the
Stripes dashboard and toggle the `Viewing test data` to `on`.

1. <ExternalLink href="https://stripe.com/">Stripe account</ExternalLink>
2. `Publishable` and `Secret` Key from the <ExternalLink href="http://dashboard.stripe.com">dashboard.stripe.com</ExternalLink>

We already have pre-build components such as the `CheckoutForm.js` in our
`e-commerce-starter` app, the two components that aren't finished and we will be
finishing them in this tutorial, are the `CheckoutForm.js` and the `Layout.js` components.

The `CheckoutForm.js` component has the `billingDetails` and an empty spot
for the `CardElement` we are going to add together, it has a submit button and a
